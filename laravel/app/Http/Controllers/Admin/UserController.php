...
class UserController extends Controller
{
  ...
  public function create(){
   ...
   public function store(Request $request){
     ...
     $b = $request->all();
     // dd($b);
        $t = \DB::table('user')->insertGetId([ 'name'=>$b['name'],'sex'=>$b['sex'],'age'=>$b['age'],'password'=>$b['password']]);
        if ($t > 0){
            return redirect("/admin/user");
        }else{
            echo '添加信息失败';
    ...
    //      $xg = \DB::table('user')->where('id','=',$id)->update(['name'=>$_POST['name'],'sex'=>$_POST['sex'],'age'=>$_POST['age']]);
    //  //如果修改成功跳转到主界面;
    //  if($xg>0){
    //      return $this->index();
    //  }else{
    ...
    // }
      public function update($id,Request $request){
        $a = $request->all();
        $xg = \DB::table('user')->where('id','=',$id)->update(['name'=>$a['name'],'sex'=>$a['sex'],'age'=>$a['age']]);
        //如果修改成功跳转到主界面;
        if($xg>0){
            // return $this->index();
            return redirect("/admin/user");
